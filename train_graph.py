...
from dataset.data import HCPGraphDataset
...
from fusion_model.GCNModel import GCNModel
...
def config():
    Config=dict()
    Config['data_root'] = '/Datasets/recogbio/'
    Config['task_id'] = 0
    Config['mode'] = 'train'
...
def experiment(Config):
    ...
    train_dataset = HCPGraphDataset(Config['data_root'], Config['task_id'], mode='train')
    val_dataset = HCPGraphDataset(Config['data_root'], Config['task_id'], mode='test')
    ...
    model = GCNModel(model).to(Config['device'])
